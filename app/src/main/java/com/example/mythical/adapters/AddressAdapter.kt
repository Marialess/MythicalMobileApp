...
import com.example.mythical.databinding.AddressRvItemBinding
...
class AddressAdapter: Adapter<AddressAdapter.AddressViewHolder>() {
    ...
    inner class AddressViewHolder(val binding: AddressRvItemBinding): ViewHolder(binding.root){
        fun bind(address: Address, isSelected: Boolean){
            binding.apply {
                buttonAddress.text = address.addressTittle
                if (isSelected){
                    buttonAddress.background = ColorDrawable(itemView.context.resources.getColor(R.color.g_gray700))
                } else {
